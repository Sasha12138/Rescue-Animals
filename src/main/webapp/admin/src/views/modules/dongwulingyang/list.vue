		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 30px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","borderRadius":"3px","background":"#fff","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">领养人</label>
						<el-input v-model="searchForm.lingyangren" placeholder="领养人" clearable></el-input>
					</div>
				...
				<el-row :style='{"margin":"20px 0 20px 0","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(64, 158, 255, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('dongwulingyang','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 0, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('dongwulingyang','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>


					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 0, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('dongwulingyang','审核')" :disabled="dataListSelections.length <= 0" type="danger" @click="shBatchDialog()">批量审核</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="lingyangzhanghao"
					label="领养账号">
						<template slot-scope="scope">
							{{scope.row.lingyangzhanghao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="lingyangren"
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('dongwulingyang','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
  ...
  methods: {
      ...
      let params = {
        ...
        order: 'desc',
          ...
          if(this.searchForm.sfsh!='' && this.searchForm.sfsh!=undefined){
           ...
           if(this.searchForm.lingyangren!='' && this.searchForm.lingyangren!=undefined){
            params['lingyangren'] = '%' + this.searchForm.lingyangren + '%'
          }
          if(this.searchForm.sfsh!='' && this.searchForm.sfsh!=undefined){
            params['sfsh'] = this.searchForm.sfsh
          }
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          lingyangzhanghao: row.lingyangzhanghao,
          lingyangren: row.lingyangren,
    ...
    shBatchHandler(){
      this.$confirm(`确定一键审核操作?`, "提示", {
        ...
        this.$http({
          url: "dongwulingyang/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
