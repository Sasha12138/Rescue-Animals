	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="动物种类" prop="dongwuzhonglei">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="动物性别" prop="dongwuxingbie">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="身体状况" prop="shentizhuangkuang">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="身体状况" prop="shentizhuangkuang">
					<el-input v-model="ruleForm.shentizhuangkuang"
						placeholder="身体状况" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="现在地址" prop="xianzaidizhi">
					<el-input v-model="ruleForm.xianzaidizhi" placeholder="现在地址" clearable  :readonly="ro.xianzaidizhi"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="现在地址" prop="xianzaidizhi">
					<el-input v-model="ruleForm.xianzaidizhi" placeholder="现在地址" readonly></el-input>
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='xianzaidizhi'){
							this.ruleForm.xianzaidizhi = obj[o];
							this.ro.xianzaidizhi = true;
							continue;
						}
						if(o=='fabushijian'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../ssm329z2/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dongwuzhaojia/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dongwuzhaojiaCrossAddOrUpdateFlag = false;
						 this.parent.search();
